...
#define DEBUGPRINT 0 // Normal 0, Debug Print 1
// TODO NUM CICLO FORK; ESTRUTURA GERIR MEM; PRINT ESTADOS DEBUG, NORMAL
...
pcb *fork_process(void **mem, pcb **p)
{
    int i;
    int j = 0;
    ...
    if (!DEBUGPRINT) {
        printf("Process %d loaded to memory from fork\n", aux->pid);
        print_memory(mem);
        printf("-----------------------------------\n");
    }
    ...
    while (!all_done) {
        ...
        if (run->num_processes > 0) {
            ...
            if (aux == 1) {
                ...
                add_process(&ready, fork_process(MEM, &temp_pcb));
                num_pcs++;
            }
